...
def create_access_token(data: dict) -> str:
    ...
    # Tiempo de expiración usando hora UTC actual
...
def get_current_user_email(token: str = Depends(oauth2_scheme)) -> str:
    ...
    try:
        # Descifrar el token usando la misma clave secreta y algoritmo
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])

        # 'sub' (subject) es donde guardamos el correo al hacer el login
